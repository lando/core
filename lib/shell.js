...
const exec = (cmd, opts) => new Promise(resolve => {
  ...
  _shell.exec(cmd.join(' '), opts, (code, stdout, stderr) => {
    console.log('exec', code, stdout, stderr);
  ...
  return new Promise(resolve => {
    if (stdio === 'pipe' || stdio[1] === 'pipe') {
      run.stdout.on('data', buffer => {
        console.log('sh - stdout', String(buffer));
    ...
    if (stdio === 'pipe' || stdio[2] === 'pipe') {
      run.stderr.on('data', buffer => {
        console.log('sh - sterr', String(buffer));
    ...
    run.on('error', buffer => {
      console.log('sh - error', String(buffer));
      stderr = stderr + String(buffer);
    });
    run.on('close', code => {
      console.log('sh - close', code, stdout, stderr);
      resolve({code, stdout, stderr});
    });
  });
};
