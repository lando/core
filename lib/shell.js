...
module.exports = class Shell {
   ...
   * });
   */
  sh(cmd, {mode = 'exec', detached = false, cwd = process.cwd(), cstdio = 'inherit', silent = false, env = process.env} = {}) {
    ...
    return Promise.try(() => {
      ...
      if (mode === 'exec' && detached === false) {
        ...
        return exec(cmd, _.merge({}, {silent: true}, {cwd, detached, mode, env}));
      }

      // Determine stdio
      const stdio = (process.lando === 'node') ? {stdio: cstdio} : {stdio: ['ignore', 'pipe', 'pipe']};
      // Get the run spawn so we can add it
      const run = child.spawn(_.first(cmd), _.tail(cmd), _.merge({}, {detached, cwd, env}, stdio));
      // Add a record of this process while its running
