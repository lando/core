...
/*
 * Helper function to get the service name from a container name
 */
exports.serviceFromContainerName = (app, container) => {
  const separator = app._config.composeSeparator || '-';
  const regex = new RegExp(`^${app.project}${separator}(.*)${separator}1$`);
  return container.replace(regex, '$1');
};
