...
class BackendManager {
  ...
  _createContainerdEngine(id) {
    ...
    const removeStaleComposeNetworks = require('../utils/remove-stale-compose-networks');
    /** @type {Object} Shared env vars for all compose invocations */
    const composeEnv = {
      ...process.env,
    ...
    const removeComposeCniConflists = require('../utils/remove-compose-cni-conflists');
    const compose = async (cmd, datum) => {
      if (cmd === 'start') {
        // Two-phase start for containerd backend:
        //
        // finch-daemon has two interacting issues:
        // 1. It auto-creates Docker API networks (without compose labels) when
        //    it discovers CNI conflist files. docker-compose v2 then rejects
        //    these: "network was not created by compose".
        // 2. When creating a network, it writes CNI conflist files that include
        //    the portmap plugin. The portmap plugin fails on HostPort:0 (random
        //    port), which Docker handles via port allocation before container start.
        //
        // Fix: remove conflist files and stale networks so docker-compose can
        // create networks fresh (with proper labels), then overwrite the
        // conflist files to remove portmap before starting containers.

        // Phase 0: Remove CNI conflist files and Docker API networks so
        // docker-compose sees a clean slate. The conflist files cause
        // finch-daemon to report ghost networks that lack compose labels.
        removeComposeCniConflists(datum.compose, datum.project, {debug: this.debug});
        await removeStaleComposeNetworks(rawDocker.dockerode, datum.project, this.debug);

        // Phase 1: Create networks, volumes, and containers without starting.
        // docker-compose creates networks via finch-daemon (which writes
        // conflist files WITH compose labels but also WITH portmap).
        this.debug('containerd compose phase 1: creating networks and containers (--no-start)');
        const createOpts = {...(datum.opts || {}), noStart: true, background: false};
        const createRun = dockerCompose['start'](datum.compose, datum.project, createOpts);
        await this.shell.sh([orchestratorBin].concat(createRun.cmd), {
          ...(createRun.opts || {}),
          env: {...composeEnv, ...(createRun.opts?.env || {})},
        });

        // Phase 2: Overwrite conflist files to remove portmap plugin.
        // finch-daemon wrote conflists with portmap during Phase 1.
        // Our ensureCniNetwork overwrites them with the correct plugin chain
        // (bridge → firewall → tuning, no portmap). The compose labels and
        // subnet/bridge config written by finch-daemon are replaced with our
        // standard config, but that's fine — the OCI hook only needs the
        // plugin chain and network name/subnet.
        this.debug('containerd compose phase 2: overwriting conflist files (removing portmap)');
        ensureComposeCniNetworks(datum.compose, datum.project, {debug: this.debug});

        // Phase 3: Start the created containers. OCI hooks now read our
        // conflist (without portmap) instead of finch-daemon's.
        this.debug('containerd compose phase 3: starting containers');
        const startRun = dockerCompose[cmd](datum.compose, datum.project, datum.opts || {});
        return this.shell.sh([orchestratorBin].concat(startRun.cmd), {
          ...(startRun.opts || {}),
          env: {...composeEnv, ...(startRun.opts?.env || {})},
        });
      }
