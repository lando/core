 ...
 *
 * @deprecated This class is **not used in production**. The containerd engine path
 * uses `docker-compose` pointed at finch-daemon via `DOCKER_HOST` instead.
 * See `BackendManager._createContainerdEngine()` in `lib/backend-manager.js`.
 ...
 * ```
 * nerdctl --address /run/lando/containerd.sock compose --project-name myapp --file docker-compose.yml up --detach ...
 * ```
 *
 * So we delegate to `compose.*()` for all the complex flag-mapping and option-parsing
 * logic, then prepend nerdctl's global connection flags to the resulting cmd array.
...
class NerdctlCompose extends ComposeBackend {
   ...
   * @param {string} [opts.socketPath='/run/lando/containerd.sock'] - Path to the
   ...
   * @param {string} [opts.namespace='default'] - containerd namespace.
   * @param {string} [opts.buildkitHost] - BuildKit endpoint for compose-triggered builds.
   * @param {string} [opts.nerdctlConfig] - Optional path to `nerdctl.toml`.
   */
  constructor(opts = {}) {
    super();
     ...
     * @type {string}
     */
    this.socketPath = opts.socketPath || '/run/lando/containerd.sock';
    this.namespace = opts.namespace || 'default';
    this.buildkitHost = opts.buildkitHost;
    this.nerdctlConfig = opts.nerdctlConfig;
   ...
   *
   * Prepends nerdctl global connection flags to the cmd array so that
   * the shell layer produces:
   *   nerdctl --address <socket> --namespace <namespace> compose <...existing args...>
   *
  ...
  _transform(result) {
    ...
    const engineEnv = {
      CONTAINERD_ADDRESS: this.socketPath,
      CONTAINERD_NAMESPACE: this.namespace,
    };
    if (this.buildkitHost) engineEnv.BUILDKIT_HOST = this.buildkitHost;
    if (this.nerdctlConfig) engineEnv.NERDCTL_TOML = this.nerdctlConfig;

    const opts = Object.assign({}, result.opts, {
      env: Object.assign({}, result.opts?.env || process.env, engineEnv, authEnv),
    });

    return {
      cmd: ['--address', this.socketPath, '--namespace', this.namespace, 'compose', ...result.cmd],
      opts,
    };
  }
