...
class ContainerdContainer extends ContainerBackend {
  ...
  constructor(opts = {}) {
    ...
    this.nerdctlBin = opts.nerdctlBin ?? path.join(userConfRoot, 'bin', 'nerdctl');

    /** @type {string} Path to the rootful containerd socket. */
    this.containerdSocket = opts.containerdSocket ?? paths.containerdSocket;

    /** @type {string} containerd namespace. */
    this.containerdNamespace = opts.containerdNamespace ?? 'default';
  ...
  async _nerdctl(args = [], opts = {}) {
    const env = Object.assign({}, process.env, {
      CONTAINERD_ADDRESS: this.containerdSocket,
      CONTAINERD_NAMESPACE: this.containerdNamespace,
    }, opts.env || {});

    const result = await runCommand(this.nerdctlBin, [
      '--address', this.containerdSocket,
      '--namespace', this.containerdNamespace,
      ...args,
    ], {debug: this.debug, env});
  ...
  async createNet(name, opts = {}) {
    ...
    return this.getNetwork(name).inspect();
  }
   ...
   */
  async scan(cid) {
    for (const id of _.uniq([cid, _.isString(cid) ? cid.replace(/_/g, '-') : cid])) {
      try {
        return await this.dockerode.getContainer(id).inspect();
      } catch (err) {
        if (!this._isNotFoundError(err)) throw err;
      }
    }

    throw new Error(`no such container: ${cid}`);
  }
  ...
  getNetwork(id) {
    return {
      ...
      remove: async () => {
        try {
          await this.dockerode.getNetwork(id).remove();
        } catch (err) {
          if (this._isNotFoundError(err)) {
      ...
      connect: (connectOpts = {}) => {
        ...
        return this.dockerode.getNetwork(id).connect(connectOpts);
      },
      ...
      disconnect: async (disconnectOpts = {}) => {
        ...
        try {
          await this.dockerode.getNetwork(id).disconnect(disconnectOpts);
        } catch (err) {
  ...
  async listNetworks(opts = {}) {
    try {
      let networks = await this.dockerode.listNetworks();

      const filters = opts.filters || {};
      if (filters.name) networks = networks.filter(network => filters.name.some(name => (network.Name || '').includes(name)));
      if (filters.id) networks = networks.filter(network => filters.id.some(id => (network.ID || network.Id || '').startsWith(id)));
      if (filters.label) {
        networks = networks.filter(network => {
          const labels = network.Labels || {};
          return filters.label.some(label => {
            if (_.isString(labels)) return labels.includes(label);
            return Object.entries(labels).some(([key, value]) => `${key}=${value}` === label);
          });
        });
      }
