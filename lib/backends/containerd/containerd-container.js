...
const parseLabels = labels => {
  ...
  for (const segment of segments) {
    if (segment.includes('=') && pairs.length === 0) {
      pairs.push(segment);
    } else if (!segment.includes('=') && pairs.length > 0) {
...
class ContainerdContainer extends ContainerBackend {
  ...
  async _nerdctl(args, opts = {}) {
    const fullArgs = ['--address', this.socketPath, ...args];
    this.debug('nerdctl %o', fullArgs);

    // Merge auth env vars (e.g. DOCKER_CONFIG) into command environment
    const authEnv = this.authConfig && this.authConfig.env ? this.authConfig.env : {};
    const hasAuthEnv = Object.keys(authEnv).length > 0;
    const mergedOpts = hasAuthEnv
      ? Object.assign({}, opts, {env: Object.assign({}, opts.env || process.env, authEnv)})
      : opts;
   ...
   *
   * Creates an **internal** network with the Lando container label, matching
   * the Docker implementation behavior. nerdctl supports `--internal` natively.
   *
  ...
  async createNet(name, opts = {}) {
    ...
    // Make it internal by default (matching Docker backend)
