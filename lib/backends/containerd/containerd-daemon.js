 ...
 * | `/run/lando/containerd/`           | containerd state directory    |
...
class ContainerdDaemon extends DaemonBackend {
  ...
  constructor(opts = {}) {
    ...
    /** @type {string} containerd --state directory (under /run/, ephemeral). */
    this.stateDir = opts.stateDir ?? '/run/lando/containerd';
   ...
   *
   * NOTE: `stateDir` (`/run/lando/containerd`) is intentionally excluded — it
   * lives under `/run/lando/` which is root-owned (created by systemd
   * `RuntimeDirectory=lando`).  The containerd daemon itself creates the state
   * subdirectory when it starts as root.  User-land code must never attempt to
   * mkdir inside `/run/lando/`.
   *
   * @private
   */
  _ensureDirectories() {
    for (const dir of [this.rootDir, this.logDir, this.configDir]) {
