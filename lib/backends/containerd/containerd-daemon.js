 ...
 * - **macOS (darwin)**: requires a Linux VM via Lima — **not yet implemented**.
...
class ContainerdDaemon extends DaemonBackend {
  ...
  async down() {
    ...
    // On macOS/Windows without VM support, this is a no-op for now
    if (this.platform === 'darwin' || this.platform === 'win32') {
      await this.events.emit('post-engine-down');
      return;
    }

    // Stop buildkitd first, then containerd
  ...
  async isUp(cache, docker) {
    ...
    // Health check: run nerdctl ps to verify connectivity
    try {
  ...
  _assertPlatformSupported() {
    if (this.platform === 'darwin') {
      // TODO: Lima VM integration for macOS
      // When implemented, this will:
      //   1. Start a Lima VM with containerd enabled
      //   2. Forward the containerd socket from the VM to the host
      //   3. Set this.socketPath to the forwarded socket
      throw new Error(
        'containerd engine on macOS requires Lima VM integration — not yet implemented. ' +
        'Please use the Docker backend on macOS for now.',
      );
    }
   ...
   */
  _ensureDirectories() {
    for (const dir of [this.runDir, this.stateDir, this.rootDir, this.logDir]) {
  ...
  async _startBuildkitd(password) {
    const args = [
      '--addr', `unix://${this.buildkitSocket}`,
      '--containerd-worker-addr', this.socketPath,
      // Disable the OCI worker since we're using the containerd worker
      '--oci-worker', 'false',
