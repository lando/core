 ...
 * | `~/.lando/run/containerd.sock`    | containerd gRPC socket        |
 * | `~/.lando/run/buildkitd.sock`     | buildkitd gRPC socket         |
...
class ContainerdDaemon extends DaemonBackend {
  ...
  constructor(opts = {}) {
    ...
    // Binary paths — expected at ~/.lando/bin/*
    const binDir = path.join(userConfRoot, 'bin');

    /** @type {string} Path to the containerd binary. */
    this.containerdBin = opts.containerdBin ?? path.join(binDir, 'containerd');
    ...
    this.buildkitSocket = opts.buildkitSocket ?? path.join(runDir, 'buildkitd.sock');
    ...
    this.finchDaemon = new FinchDaemonManager({
      ...
      socketPath: opts.finchDaemonSocket || path.join(runDir, 'finch.sock'),
      logDir: this.logDir,
      debug: this.debug,
    });
  }
  ...
  async up(retry = true, password) {
    ...
    const starter = async () => {
      ...
      try {
        // Try rootless containerd via systemd on Linux (no root required)
        if (this.platform === 'linux' && !password) {
          await this._startRootless();
        } else {
          // Start containerd directly (needs root on Linux)
        ...
        // Note: buildkitd needs containerd socket — in rootless mode it uses
        // the rootless namespace socket, so we skip managed buildkitd for now
        // and rely on nerdctl's built-in buildkit support
        if (!this.useRootless) {
  ...
  async isUp(cache, docker) {
    ...
    // Try rootless containerd first (nerdctl auto-detects without --address)
    try {
      await require('../../../utils/run-command')(
        docker,
        ['ps'],
        {debug: this.debug},
      );
      this.debug('containerd engine is up (rootless auto-detected).');
      this.useRootless = true;
      cache.set('containerd-engineup', true, {ttl: 5});
      this.isRunning = true;
      return Promise.resolve(true);
    } catch {
      // Fall through to socket-based check
    }

    // Check explicit socket path
    if (!fs.existsSync(this.socketPath)) {
      this.debug('containerd is down: socket does not exist at %s', this.socketPath);
      return Promise.resolve(false);
    }

    // Health check with explicit socket
    try {
      await require('../../../utils/run-command')(
        docker,
        ['--address', this.socketPath, 'ps'],
        {debug: this.debug},
      );
      this.debug('containerd engine is up.');
      cache.set('containerd-engineup', true, {ttl: 5});
      this.isRunning = true;
      return Promise.resolve(true);
    } catch (error) {
  ...
  /**
   * Start containerd in rootless mode via systemd user service.
   *
   * Uses nerdctl's `containerd-rootless-setuptool.sh` which creates and manages
   * a systemd user service running containerd under rootlesskit. This avoids
   * the need for root permissions on Linux.
   *
   * @returns {Promise<void>}
   * @private
   */
  async _startRootless() {
    const runCommand = require('../../../utils/run-command');
    const binDir = path.dirname(this.containerdBin);
    const setupScript = path.join(binDir, 'containerd-rootless-setuptool.sh');
    const env = {...process.env, PATH: `${binDir}:/usr/sbin:/sbin:${process.env.PATH || ''}`};

    // Check if systemd user service is already running
    try {
      await runCommand('systemctl', ['--user', 'is-active', '--quiet', 'containerd.service'], {
        debug: this.debug,
      });
      this.debug('rootless containerd already running via systemd');
      this.useRootless = true;
      return;
    } catch {
    ...
    if (!fs.existsSync(setupScript)) {
      throw new Error(
        'containerd-rootless-setuptool.sh not found. Run "lando setup" to install containerd.',
      );
    }

    // Try starting existing service first
    try {
      await runCommand('systemctl', ['--user', 'start', 'containerd.service'], {
        debug: this.debug,
      });
      this.debug('started existing rootless containerd service');
      this.useRootless = true;
      ...
      for (let i = 0; i < 10; i++) {
        try {
          await runCommand(this.nerdctlBin, ['ps'], {debug: this.debug, env});
          return;
        } catch {
          await delay(500);
        }
      ...
      // Service doesn't exist yet, install it
    }

    // Install rootless containerd
    this.debug('installing rootless containerd via setup script');
    try {
      await runCommand('bash', [setupScript, 'install'], {
        debug: this.debug,
        env,
      });
      this.debug('rootless containerd installed and started');
      this.useRootless = true;
    } catch (error) {
      throw new Error(`Failed to install rootless containerd: ${error.message}`);
    }
  }
  // =========================================================================
  ...
  async _healthCheck() {
    const runCommand = require('../../../utils/run-command');
    const args = this.useRootless ? ['ps'] : ['--address', this.socketPath, 'ps'];
    await runCommand(this.nerdctlBin, args, {debug: this.debug});
  }
