...
class ContainerdDaemon extends DaemonBackend {
  ...
  async _startContainerd(password) {
    ...
    // On WSL, write a custom containerd config to avoid conflicts with Docker Desktop
    if (this.wslHelper) {
      await this.wslHelper.writeConfig(this.configPath, this.socketPath, this.stateDir, this.rootDir);
