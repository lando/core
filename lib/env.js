 ...
 */
exports.getComposeExecutable = ({userConfRoot = os.tmpdir()} = {}) => {
  switch (process.platform) {
    case 'darwin':
    case 'linux':
      // use lando bin if available
      if (fs.existsSync(path.join(userConfRoot, 'bin', 'docker-compose'))) {
        return getDockerBin('docker-compose', path.join(userConfRoot, 'bin'), false);
      }
      // otherwise use docker desktop one if available
      return getDockerBin('docker-compose', exports.getDockerComposeBinPath(), process.platform === 'linux');

    case 'win32':
      // use lando bin if available
      if (fs.existsSync(path.join(userConfRoot, 'bin', 'docker-compose-v1.exe'))) {
        return getDockerBin('docker-compose-v1', path.join(userConfRoot, 'bin'), false);
      }
      // otherwise use docker desktop one if available
