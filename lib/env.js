 ...
 */
exports.getComposeExecutable = ({composeVersion = '2.21.0', userConfRoot = os.tmpdir()} = {}) => {
  const composeBin = `docker-compose-v${composeVersion}`;
  switch (process.platform) {
    case 'darwin':
    case 'linux':
      // use lando bin if available
      if (fs.existsSync(path.join(userConfRoot, 'bin', composeBin))) {
        return getDockerBin(composeBin, path.join(userConfRoot, 'bin'), false);
      }
    ...
    case 'win32':
      // use lando bin if available
      if (fs.existsSync(path.join(userConfRoot, 'bin', `${composeBin}.exe`))) {
        return getDockerBin(composeBin, path.join(userConfRoot, 'bin'), false);
      }
      // otherwise use docker desktop one if available
