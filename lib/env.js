...
exports.getComposeExecutable = ({composeVersion = '2.21.0', userConfRoot = os.tmpdir()} = {}) => {
  switch (process.platform) {
    ...
    case 'linux':
      ...
      if (fs.existsSync(path.join(userConfRoot, 'bin', 'docker-compose-v2'))) {
        return getDockerBin('docker-compose-v2', path.join(userConfRoot, 'bin'), false);
      }
    ...
    case 'win32':
      // use lando bin if available
      if (fs.existsSync(path.join(userConfRoot, 'bin', 'docker-compose-v2.exe'))) {
        return getDockerBin('docker-compose-v2', path.join(userConfRoot, 'bin'), false);
      }
      // otherwise use docker desktop one if available
