...
module.exports = class Engine {
  ...
  constructor(daemon = new LandoDaemon(), docker = new Landerode(), compose = () => {}, config = {}) {
    ...
    this.composeInstalled = fs.existsSync(config.composeBin);
    this.dockerInstalled = this.daemon.docker !== false;

    // set the compose separator
    this.separator = _.get(config, 'composeSeparator', '_');
  ...
  getCompatibility(supportedVersions = this.supportedVersions) {
    ...
    return this.daemon.getVersions().then(versions => {
      ...
      return _(versions).map((version, name) => ({
        name,
        link: supportedVersions[name].link[process.platform],
        satisfied: semver.satisfies(semver.clean(version), supportedVersions[name].satisfies),
        dockerVersion: true,
  ...
  list(options = {}, separator = this.separator) {
    return this.engineCmd('list', options, options => this.docker.list(options, this.separator));
  };
