...
module.exports = class Engine {
  ...
  constructor(daemon = new LandoDaemon(), docker = new Landerode(), compose = () => {}, config = {}) {
    ...
    // engine backend indicator
    this.engineBackend = config.engine || 'auto';

    // Determine install status
    // When engine is containerd, dockerInstalled reflects containerd availability
    if (this.engineBackend === 'containerd') {
      this.composeInstalled = fs.existsSync(config.orchestratorBin);
      this.dockerInstalled = this.daemon.containerd !== false
        && fs.existsSync(this.daemon.containerd);
    } else {
      this.composeInstalled = fs.existsSync(config.orchestratorBin);
    ...
    // Supported version ranges for containerd backend
    this.supportedContainerdVersions = config.supportedContainerdVersions;
   ...
   */
  destroy(data) {
    // // if no services then just return true
    // // @TODO: we need this for DC2 because, unlike DC1 it fails when there are no services
    if (_.has(data, 'opts.services') && _.get(data, 'opts.services', []).length === 0) return Promise.resolve();
  ...
  getCompatibility(supportedVersions = this.supportedVersions) {
    ...
    const normalize = sv => _(sv)
      ...
      .map(data => ([data.name, {
        ...
        link: _.isObject(data.link) && !_.isArray(data.link) ? (data.link[this.platform] || data.link) : data.link,
    ...
    return this.daemon.getVersions().then(versions => {
      ...
      return _(versions).map((version, name) => {
        const reqs = normalizedVersions[name];
        // skip versions we don't have supported ranges for
        if (!reqs) return null;
        return {
          name,
          link: reqs.link,
   ...
   * @param {String} [separator='_'] The docker compose separator to use
   ...
   * });
   */
  list(options = {}, separator = this.separator) {
    return this.engineCmd('list', options, options => this.docker.list(options, separator));
  }
  ...
  start(data) {
    ...
    // otherwise we should be able to do something?
