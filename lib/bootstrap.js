...
exports.buildConfig = options => {
  ...
  // If composeBin is set, is an absolute path and exists then unset composeVersion and rely on this alone
  if (typeof config.composeBin === 'string'
    && path.isAbsolute(config.composeBin)
    && fs.existsSync(config.composeBin)) {
    delete config.composeVersion;
  // Otherwise remove composeBin and rely on composeVersion alone
  } else {
    delete config.composeBin;
  }
  ...
  // If composeSeparator is set to '-' and we are using docker-compose 2 then allow that
  config.env.COMPOSE_COMPATIBILITY = config.composeSeparator === '_';
...
exports.setupEngine = (config, cache, events, log, shell, id) => {
  ...
  // get enginey stuff
  const {composeBin, composeVersion, dockerBin, engineConfig} = config;
  const docker = new Landerode(engineConfig, id);
  const daemon = new LandoDaemon(cache, events, dockerBin, log, config.process, composeBin, composeVersion);
  const compose = (cmd, datum) => exports.dc(shell, composeBin, cmd, datum);
