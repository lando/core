...
const engineRunner = (config, command) => (argv, lando) => {
  ...
  if (!task.dir && _.has(app, `mounts.${task.service}`)) task.appMount = app.mounts[task.service];
  // and working dir data if no dir or appMount
  if (!task.dir && !_.has(app, `mounts.${task.service}`) && _.has(app, `config.services.${task.service}.working_dir`)) {
    task.dir = app.config.services[task.service].working_dir;
  }
