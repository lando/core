...
module.exports = class App {
   ...
   */
  addMessage(message, error = null) {
    // Do special handling if this is a warning or error
    if (error && ['warning', 'warn', 'error'].includes(message.type)) this.addWarning(message, error);
  ...
  // @NOTE: doing something like this is basically the only way to work around the lando 3 runtime chaos
  depCheck() {
    this.log.verbose('build-engine exists: %s', this.engine.dockerInstalled);
    this.log.verbose('orchestrator exists: %s', this.engine.composeInstalled);

    // BUILD ENGINE
    if (this.engine.dockerInstalled === false) {
      if (this._lando.cli) {
        const dep = process.platform === 'linux' ? 'DOCKER ENGINE' : 'DOCKER DESKTOP';
        console.error(this._lando.cli.makeArt('noDockerDep', dep));
        process.exit(1);
      } else throw Error('docker could not be located!');
    }

    // ORCHESTRATOR
    if (this.engine.composeInstalled === false) {
      if (this._lando.cli) {
        console.error(this._lando.cli.makeArt('noDockerDep', 'DOCKER COMPOSE'));
        process.exit(1);
