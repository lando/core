...
module.exports = class App {
  ...
  init({noEngine = false} = {}) {
    ...
    .then(() => {
      ...
      // add a double property because we have weird differnces between cli getApp and core getApp
    ...
    .then(() => {
      ...
      _.forEach(globalsData.services, (service, name) => {
        ...
        service.extra_hosts = ['host.lando.internal:host-gateway'];

        if (_.get(this, 'config.keys', true)) {
          service.volumes.push(
            `${path.join(this._config.home, '.ssh')}:/user/.ssh`,
          );
        }

        if (undefined !== this.config.services[name] && false === this.config.services[name].ssl) {
          return;
        }

        // certs
        const certname = `${name}.${this.project}.crt`;
        const keyname = `${name}.${this.project}.key`;
        service.environment.LANDO_SERVICE_CERT = `/lando/certs/${certname}`;
        service.environment.LANDO_SERVICE_KEY = `/lando/certs/${keyname}`;
        service.volumes.push(
          `${path.join(this._config.userConfRoot, 'scripts', 'add-cert.sh')}:/scripts/000-add-cert.sh`,
        );
        service.volumes.push(`${path.join(this._config.userConfRoot, 'certs', certname)}:/certs/cert.crt`);
        service.volumes.push(`${path.join(this._config.userConfRoot, 'certs', keyname)}:/certs/cert.key`);
      });
 ...
 /**
  ...
  async runTasks(tasks, options = {}) {
    ...
    if (options.renderer === 'dc2') {
      options = _.merge({}, {debug: 'dc2-debug', fallbackRenderer: 'dc2-fallback'}, options);
    }
