...
exports.run = (compose, project, opts = {}) => {
  // Compute TTY at call time (not module load time) so that the
  // check reflects the actual terminal state when the command runs.
  // Callers can still override by passing noTTY explicitly.
  if (opts.noTTY === undefined) {
