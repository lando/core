...
module.exports = class Lando {
  ...
  bootstrap(level = 'app') {
    ...
    // autosetup if applicable
    .then(async () => this.config.setup.auto ? await this.setup(this.config.setup) : {})
  ...
  // setup
  async setup(options = this.config.setup, tasks = [], results = {}) {
    // pre setup event to mutate options
    await this.events.emit('pre-setup', options, tasks);
