...
const bootstrapEngine = lando => {
  ...
  // Use BackendManager to create the engine (replaces setup-engine.js)
  // lando.engine = require('../utils/setup-engine')(
  //   lando.config, lando.cache, lando.events, lando.log, lando.shell, lando.config.instance,
  // );
  const BackendManager = require('./backend-manager');
  const backendManager = new BackendManager(lando.config, lando.cache, lando.events, lando.log, lando.shell);
  lando.engine = backendManager.createEngine(lando.config.instance);
  lando.backendManager = backendManager;

  // When using containerd, update global orchestratorBin to docker-compose
  // (used via finch-daemon) so lando-reset-orchestrator.js doesn't replace
  // the engine with Docker's setup-engine.
  if (lando.engine.engineBackend === 'containerd') {
    const composeVersion = lando.config.orchestratorVersion || '2.31.0';
    const composeBin = path.join(lando.config.userConfRoot, 'bin', `docker-compose-v${composeVersion}`);
    if (fs.existsSync(composeBin)) {
      lando.config.orchestratorBin = composeBin;
    }

    const finchSocket = lando.engine.daemon.finchDaemon.getSocketPath();
    lando.config.dockerBin = lando.engine.daemon.nerdctlBin;
    lando.config.engineConfig = {
      ...lando.config.engineConfig,
      authConfig: getContainerdAuthConfig({configPath: lando.config.registryAuth}),
      buildkitHost: `unix://${lando.engine.daemon.buildkitSocket}`,
      containerdMode: true,
      containerdNamespace: 'default',
      containerdSocket: lando.engine.daemon.socketPath,
      engine: 'containerd',
      nerdctlConfig: path.join(lando.config.userConfRoot, 'config', 'nerdctl.toml'),
      socketPath: finchSocket,
    };
  }

  lando.log.info('engine backend: %s', lando.config.engine || 'auto');
...
module.exports = class Lando {
  ...
  async getSetupStatus(options = this.config.setup) {
    ...
    const results = await Promise.all(options.tasks
      .filter(task => task.hidden !== true)
      .map(async task => {
        // extract status fields with defaults — intentionally NOT calling parse-setup-task here
        // because that mutates/wraps the task object and setup() needs to do that exactly once
        const slugify = require('slugify');
        const id = task.id ?? slugify(task.title);
        const canRun = task.canRun ?? (async () => true);
        const comments = task.comments ?? {};
        const description = task.description ?? task.title;
        const hasRun = task.hasRun ?? (async () => false);
        const requiresRestart = task.requiresRestart ?? false;
