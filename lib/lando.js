...
const bootstrapEngine = lando => {
  ...
  // Use appropriate separator for Docker Compose version.
  const dockerSeparatorPromise = lando.engine.daemon.getVersions().then(versions => {
    const semver = require('semver');
    const isComposeV1 = semver.lt(versions.compose || '1.0.0', '2.0.0');
    lando.config.composeSeparator = isComposeV1 ? '_' : '-';
  });
  ...
  const pluginsPromise = lando.Promise.map(lando.config.plugins, plugin => {
  ...
  return lando.Promise.all([dockerSeparatorPromise, pluginsPromise]);
};
...
const bootstrapApp = lando => {
  ...
  const legacyItems = _(['builders'])
    ...
    .map(file => lando.factory.add(file).name)
    .value();
  _.forEach(legacyItems, builder => lando.log.debug('autodiscovered legacy builder %s', builder));
  ...
  // @TODO: factory.add should allow for "custom" metadata to get merged in eg {api: 4ish}?
};
