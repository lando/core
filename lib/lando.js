...
const bootstrapEngine = lando => {
  ...
  // When using containerd, update global orchestratorBin to nerdctl so
  // lando-reset-orchestrator.js doesn't replace the engine with Docker
  if (lando.engine.engineBackend === 'containerd' && lando.engine.daemon && lando.engine.daemon.nerdctlBin) {
    lando.config.orchestratorBin = lando.engine.daemon.nerdctlBin;
  }
