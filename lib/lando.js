...
module.exports = class Lando {
  ...
  async setup(options = this.config.setup) {
    ...
    // collect our things
    const data = {errors: [], results: [], total: 0};
    ...
    if (options.installTasks) {
      // resolve our tasks
      const tasks = resolveSetupTasks(options.tasks.map(task => require('../utils/parse-setup-task')(task)));
      // and then run them
      const {errors, results, total} = await this.runTasks(tasks,
        {
          concurrent: true,
          exitOnError: false,
          renderer: 'lando',
      ...
      data.results = [...data.results, ...results];
      data.total = data.total + total;
      this.log.debug('completed %s of %s tasks with %s errors', results.length, total, errors.length);
    }
  ...
  async getSetupStatus(options = this.config.setup) {
    ...
    const results = await Promise.all(options.tasks.map(async task => {
      // break it up
      const {canRun, comments, description, hasRun, version} = require('../utils/parse-setup-task')(task);
      // lets start optimistically
