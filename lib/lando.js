...
module.exports = class Lando {
  ...
  async installPlugins(options = this.config.setup) {
    ...
    this.log.debug('installed %s plugins with %s errors', results.added, results.errors.length);
  ...
  async setup(options = this.config.setup) {
    ...
    // collect our things
    const data = {errors: [], results: [], pluginsInstalled: 0, tasksCompleted: 0};
    ...
    if (options.installTasks) {
      const {errors, results, run} = await this.runTasks(resolveSetupTasks(options.tasks), {
        ctx: {run: 0},
        renderer: 'lando',
        rendererOptions: {
          level: 0,
      ...
      data.errors = [...data.errors, ...errors];
      data.results = [...data.results, ...results];
      data.tasksCompleted= run;
    }

    // post setup event
    await this.events.emit('post-setup', data);

    this.log.debug('ran %s tasks with %s errors', data.tasksCompleted, data.errors.length);
  ...
  async getSetupStatus(options = this.config.setup) {
    ...
    const results = await Promise.all(options.tasks.map(async task => {
      // break it up
      const {canRun, description, hasRun, version} = task;
      // lets start optimistically
