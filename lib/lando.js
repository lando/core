...
// Helper to run setup tasks
// const getSetupListr = (tasks = []) => ([
//   [{
//     title: 'Running Setup?',
//     task: (ctx, task) => {
//       return task.newListr(tasks, {concurrent: true, exitOnError: false});
//     },
//   }],
//   {renderer: 'lando', rendererOptions: {level: 0}},
// ]);
...
module.exports = class Lando {
  ...
  async getInstallPluginsStatus(options = this.config.setup) {
    ...
    const results = await Promise.all(plugins.map(async plugin => {
      ...
      const status = {description, state: 'INSTALLED', plugin};
  ...
  async setup(options = this.config.setup) {
    ...
    // collect our things
    const data = {errors: [], results: [], pluginsAdded: 0, tasksCompleted: 0};

    // if we should add plugins then install them
    if (options.installPlugins) {
      const {errors, results, added} = await this.installPlugins(options);
      data.errors = [...data.errors, ...errors];
      data.results = [...data.results, ...results];
      data.pluginsAdded = added;
    ...
    // reset plugins with bootstrapConfig?
    // recurse setup if sucesss (this should rerun the hooks and skip plugin install)

    // run setup if we have tasks
    // if (!_.isEmpty(tasks)) {
    //   this.log.debug('running setup with', options, tasks);
    //   results = await this.runTasks(...getSetupListr(tasks));
    //   this.log.debug('setup finished with', results);
    //   // write "first time setup file" if it doesnt exist?
    // }

    // post setup event
    await this.events.emit('post-setup', results, options, tasks);
    // return the results
    return results;
  }
};
