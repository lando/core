...
const bootstrapApp = lando => {
  ...
  // start with legacy discovery
  // @TODO: factory changes?
  // 1. make factory order agnostic
  //    get returns a class
  //    allow get to return the "raw" registry value

  // @NOTE; *-legacy directories are intended for organizational purposes only eg you dont want service "builders"
  // to coexist in the same dir as new 4ish services
  // separate out recipes also?
  const autoScanDirs = ['compose', 'types', 'services', 'recipes'].map(dir => ([dir, `${dir}-legacy`])).flat(1);

  // Load in all our builders in the correct order
  const builders = _(autoScanDirs)
