...
const bootstrapConfig = async lando => {
  ...
  // add the plugins to the updater
...
module.exports = class Lando {
  constructor(options = {}) {
    ...
    const pluginInstallDir = this.config.pluginDirs.find(dir => dir.type === require('../utils/get-plugin-type')());
  ...
  async setup(options = this.config.setup) {
    ...
    data.restart = data.results.length > 0 && options.tasks.some(task => task.requiresRestart === true);
