...
module.exports = class Factory {
  ...
  add(item, options = {}) {
    ...
    if (_.isString(item) && fs.existsSync(item) && path.basename(item, '.js') !== 'builder') {
      this.registry.push({name: path.basename(item, '.js'), path: item});
      return this.getRaw(path.basename(item, '.js'));
    }
    ...
    this.registry.push(_.merge({}, serviceDefaults, item));
  ...
  get(name) {
    ...
    let service = _.find(this.registry, {name});
    ...
    if (_.isFunction(service.builder) && !isClass(service.builder)) {
      service.builder = service.builder(this.get(service.parent), service.config);
    }
   ...
   */
  getRaw(name) {
    return (!_.isEmpty(name)) ? _.find(this.registry, {name}) : this.registry;
  };
};
