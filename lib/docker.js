...
module.exports = class Landerode extends Dockerode {
  ...
  list(options = {}, separator = '_') {
    ...
    .map(container => require('../utils/to-lando-container')(container, separator))
    ...
    // if any container has been up for only a brief amount of time then
    .then(containers => {
    ...
    // add a "running" status to the thing
    .map(container => {
      container.running = !container.status.includes('Exited');
      return container;
    });
  };
