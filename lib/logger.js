...
module.exports = class Log extends winston.Logger {
  constructor({logDir, logLevelConsole = 'warn', logLevel = 'debug', logName = 'lando'} = {}) {
    // allow DEBUG flag to set verbose mode
    if (process && process.env && process.env.DEBUG === 'lando*') logLevelConsole = 'debug';
    if (process && process.env && process.env.DEBUG === '*') logLevelConsole = 'debug';
