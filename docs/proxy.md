...
Note that while `name` is arbitrary if it ends in `-secured` it will _only_ be applied to `https` routes. Please consult the [traefik documentation](https://doc.traefik.io/traefik/middlewares/overview/) for the exact Docker label based syntax.
