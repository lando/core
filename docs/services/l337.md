...
You can pass in [build args](https://docs.docker.com/build/guide/build-args/) that are then used by your `image` instructions. Both `array` and `object` notation is permissible but `object` is probably better. Both formats work regardless of `buildx`.
